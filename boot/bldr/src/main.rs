...
fn copy_cpuid_page(kernel_launch_info: &mut KernelLaunchInfo) {
    ...
    unsafe {
        ...
        let src = slice::from_raw_parts(CPUID_PAGE as usize as *const u8, 0x1000);
...
pub extern "C" fn bldr_main(launch_info: &BldrLaunchInfo, vtom: u64) -> ! {
    ...
    if launch_info.platform_type == u32::from(SvsmPlatformType::Snp) {
        copy_cpuid_page(kernel_launch_info);
    }
