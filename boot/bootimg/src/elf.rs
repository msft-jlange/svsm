...
use crate::page_tables::{is_page_aligned, page_align_up, KernelPageTables, PteType};
use crate::{add_page_data, round_to_pages, BootImageHost};
...
fn load_elf_segment<'a, H: BootImageHost<'a>>(
    ...
    if !is_page_aligned(segment_start) {
        host.display_error("Kernel ELF segment is not aligned");
        return Err(());
    }
    ...
    page_tables.map_range(segment_start, segment_len, paddr, pte_type, host)?;
    ...
    add_page_data(host, paddr, Some(segment.file_contents), segment_len)?;
...
pub fn load_kernel_elf<'a, H: BootImageHost<'a>>(
    ...
    if let Some(dyn_relocs) = elf
        ...
        .map_err(|_| {
            host.display_error("Failed to apply ELF relocations");
        })?
    {
        for reloc in dyn_relocs {
            let Some(reloc) = reloc.map_err(|_| {
                host.display_error("Failed to apply ELF relocations");
            })?
            else {
                continue;
