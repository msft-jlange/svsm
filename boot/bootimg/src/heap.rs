...
impl KernelPageHeap {
    fn create(phys_base: u64, page_count: u64, direct_map_base: u64, virt_base: u64) -> Self {
        Self {
            direct_map_base,
            virt_base,
            phys_base,
...
pub fn create_kernel_heap(
    ...
    // Calculate the base of the direct map.  This is the kernel base address
    // plus the span of virtual address mappings, rounded up to the next 2 MB
    // boundary.
    ...
    let heap = KernelPageHeap::create(heap_pstart, heap_size, direct_map_base, heap_vstart);
