...
use crate::{BootImageHost, PGTABLE_LVL3_IDX_PTE_SELFMAP};
...
fn pte_index(vaddr: u64, level: u64) -> u64 {
    (vaddr >> (PTE_SHIFT * level + 12)) & (PTE_PER_PAGE - 1)
}
...
impl PageTableContents {
    fn new(count: usize, confidentiality_mask: u64) -> Self {
        // Allcoation of a zeroed slice is unstable in the current version of
        // the toolchain, so instead, an unintialized alloation is made and
    ...
    fn set_entry(&mut self, index: usize, paddr: u64, pte_type: PteType) {
        *self.entry_mut(index) = X64Pte::new(paddr | self.confidentiality_mask, pte_type);
    }
...
impl KernelPageTables {
    ...
    pub fn kernel_pml4e_index(&self) -> usize {
        self.kernel_pml4e_index
    }

    pub fn map_range<'a, H: BootImageHost<'a>>(
        &mut self,
        vaddr: u64,
        mut len: u64,
        mut paddr: u64,
        ...
        if starting_pte_index < self.starting_index {
            host.display_error("Kernel virtual address out of bounds");
            return Err(());
        }
    ...
    pub fn add_to_image<'a, H: BootImageHost<'a>>(&self, host: &mut H) -> Result<u64, ()> {
        add_page_contents(host, self.starting_paddr, self.pte_allocation.as_bytes())
            .and(Ok(self.paging_root))
    }
}
...
pub fn setup_kernel_page_tables<'a, H: BootImageHost<'a>>(
    ...
    let pte_count: usize = round_to_pages(kernel_virt_len).try_into().map_err(|_| {
        host.display_error("Kernel region size is too large");
    })?;
    ...
    let pde_count = round_to_pages(end_pte_index as u64) as usize * size_of::<X64Pte>();
    ...
    if end_pde_index as u64 > PTE_PER_PAGE {
        host.display_error("Kernel virtual address span does not fit within 1 GB");
        return Err(());
    }
    ...
    let (paging_root, root_vaddr) = kernel_heap
        .allocate_pages(pde_count as u64 + 3)
        .inspect_err(|_| {
            host.display_error("Insufficient memory to allocate kernel page tables");
        })?;

    // Determine the confidentiality mask that will be used for private
    // addresses.
    let confidentiality_mask = host.get_confidential_address_mask();

    // Allocate memory to describe the page tables.
    let pte_allocation = PageTableContents::new(pte_count, confidentiality_mask);

    // Populate the upper levels of the paging hierarchy down to the page
    // directory level.
    let mut paddr = paging_root;

    let mut pxe_allocation = PageTableContents::new(PTE_PER_PAGE as usize, confidentiality_mask);
    ...
    if pxe_allocation.read_entry(pml4e_index) != 0 {
        host.display_error("Kernel address range conflicts with self-map");
        return Err(());
    }
