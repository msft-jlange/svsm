    ...
    F: FnMut(u64, Option<&[u8]>, u64) -> Result<(), BootImageError>,
{
    ...
    // the direct map.
    let kernel_virt_len = kernel_heap.direct_map_base - kernel_virt_base;
    ...
    // using large pages.
    let large_page_count = round_to_large_pages(kernel_heap.page_count() * PAGE_SIZE_4K) as usize;
    ...
    for pde_index in 0..large_page_count {
        ...
        pxe_allocation.set_entry(
            ...
            kernel_region_phys_base + offset,
            PteType::RwLarge,
        );
    }
    add_page_contents(add_page_data, paddr, pxe_allocation.as_bytes())?;
