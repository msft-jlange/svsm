...
impl PerCpuAreas {
    ...
    /// SAFETY: the areas vector obtained here is not multi-thread safe, so the
    /// caller must guarantee that is not used in a context that expects
    /// multi-thread safety.
    ...
    /// caller must guarantee that is not used in a context that expects
    /// multi-thread safety.
    unsafe fn get_areas_mut(&self) -> &mut Vec<&'static PerCpuShared> {
        // SAFETY: the caller guarantees that accessing the unsafe cell is
        // appropriate.
        unsafe { &mut *self.areas.get() }
    }

    /// SAFETY: this function can only be invoked before any other processors
    /// have started.  Otherwise, accesses to the areas vector will not be
    /// safe.
    pub unsafe fn create_new(&self, apic_id: u32) -> &'static PerCpuShared {
        ...
        let areas = unsafe { self.get_areas_mut() };
