...
impl<'a> HypercallPagesGuard<'a> {
    ...
    fn hypercall_rep_input<H, T>(&mut self) -> (&mut H, &mut [T]) {
        ...
        unsafe {
            ...
            // exclusively owned by the `HypercallPagesGuard` and can safely
            // be cast to a header of type `H` followed by an array of `T` up
            // to the size of one page.
            (
                &mut *header,
    ...
    fn hypercall_output<T>(&self, output: HvHypercallOutput) -> &[T] {
        ...
        unsafe {
            // SAFETY - the virtual address represents an entire page which is
            // exclusively owned by the `HypercallPagesGuard` and can safely
            // be cast to an array of `T` up to the size of one page.
            slice::from_raw_parts(self.output.vaddr.as_ptr::<T>(), count)
        }
    }
}
...
pub fn get_vp_register(name: hyperv::HvRegisterName) -> Result<u64, SvsmError> {
    ...
    *header = input_header;
    ...
    let call_output = unsafe {
        // SAFETY - the GetVpRegisters hypercall does not write to any memory
        // other than the hypercall page, and does not consume memory that is
        // not included in the hypercall input.
