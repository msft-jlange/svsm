...
use crate::hyperv;
use crate::hyperv::{HyperVMsr, HvInitialVpContext};
...
#[derive(Debug)]
pub struct HypercallPagesGuard {
    pub input: VirtPhysPair,
    pub output: VirtPhysPair,
    _irq_guard: IrqGuard,
}

impl HypercallPagesGuard {
    pub fn new(input: VirtPhysPair, output: VirtPhysPair) -> Self {
        Self {
            input,
            output,
    ...
    fn hypercall_input<'a, H: Sized>(&self) -> &'a mut H {
        let header = self.input.vaddr.as_mut_ptr::<H>();
        unsafe { &mut *header }
    }

    fn hypercall_rep_input<'a, H: Sized, T: Sized>(&self) -> (&'a mut H, &'a mut [T]) {
    ...
    fn hypercall_output<'a, T: Sized>(&self, output: HvHypercallOutput) -> &'a [T] {
...
fn hypercall(
    ...
    hypercall_pages: &HypercallPagesGuard,
) -> HvHypercallOutput {
...
pub fn get_vp_register(name: hyperv::HvRegisterName) -> Result<u64, SvsmError> {
    ...
    let hypercall_pages = this_cpu().get_hypercall_pages();
    let (header, slice) = hypercall_pages.hypercall_rep_input::<HvInputGetVpRegister, u32>();
    *header = input_header;
    slice[0] = name as u32;

    let call_output = hypercall(input_control, &hypercall_pages);
...
fn enable_vp_vtl_hypercall(cpu: &PerCpu, vtl: u8, context: &HvInitialVpContext) -> Result<(), SvsmError> {
    ...
    let input_control =
        HvHypercallInput::new().with_call_code(HvCallCode::EnableVpVtl as u16);
...
fn start_vp_hypercall(cpu: &PerCpu, vtl: u8, context: &HvInitialVpContext) -> Result<(), SvsmError> {
