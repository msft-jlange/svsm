...
use svsm::cpu::ipi::{send_multicast_ipi, send_unicast_ipi, IpiMessage, IpiTarget};
...
pub extern "C" fn svsm_main() {
    ...
    let message = IpiMessage::NoOperation;
    unsafe {
        send_multicast_ipi(IpiTarget::All, message);
    }

    let message = IpiMessage::NoIpiMut(4);
    let response = unsafe { send_unicast_ipi(1, message) };
    if let IpiMessage::NoIpiMut(v) = response {
        assert_eq!(v, 0);
    } else {
