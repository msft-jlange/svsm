...
use svsm::mm::{init_kernel_mapping_info, FixedAddressMappingRange, PageBox, TransitionPageTable};
use svsm::platform::{
    init_capabilities, init_platform_type, PageValidateOp, SvsmPlatform, SvsmPlatformCell,
    SVSM_PLATFORM,
};
...
unsafe fn memory_init(launch_info: &KernelLaunchInfo, platform: &dyn SvsmPlatform) {
    // Unallocated hep memory has not already been accepted so it must be
    // accepted here.
    ...
    if launch_info.heap_area_allocated < launch_info.heap_area_page_count {
        // SAFETY: the launch info is assumed to correctly
        unsafe {
            platform
